...
function initSection3() {
    ...
    function isValid() {
        ...
        if (payoffDate <= loanDate) {
            alert('La fecha de liquidación debe ser posterior a la fecha del préstamo.');
            return false;
        }
    ...
    function calculateEarlyPayoff() {
        ...
        const accruedInterest = Math.round(dailyInterest * diffDays);
        const totalPayoff = Math.round(state.amount + accruedInterest);
...
function initSection4() {
    ...
    function isValid() {
        ...
        if (currDate <= prevDate) {
            alert('La fecha de liquidación debe ser posterior a la fecha del préstamo anterior.');
            return false;
        }
    ...
    function calculateOverLoan() {
        ...
        const prevAnnualInterest = state.prevAmount * prevRate;
        const prevDailyInterest = Math.round(prevAnnualInterest / 365);
        ...
        const accruedInterest = Math.round(prevDailyInterest * daysElapsed);
        const currentPayoff = Math.round(state.prevAmount + accruedInterest);
    ...
    function renderResults() {
        ...
        card.innerHTML = `
            ...
            <div class="result-row">
                <span>Interés Acumulado:</span>
                <span>$${data.accruedInterest.toFixed(2)}</span>
            </div>
            <div class="result-row">
